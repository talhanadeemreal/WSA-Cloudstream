...
If ($(Get-WindowsOptionalFeature -Online -FeatureName 'VirtualMachinePlatform').State -Ne "Enabled") {
    ...
    Write-Warning "Enabling Virtual Machine Platform. A restart will be required after installation."
}
...
$Dependencies | ForEach-Object {
    ...
    If ( $InstalledVersion -Lt $_.MinVersion ) {
        If ($env:WT_SESSION) {
            $env:WT_SESSION = $null
            Write-Output "Dependency should be installed but Windows Terminal is in use."
            # Cannot restart to conhost in non-interactive mode easily, hoping it works or failing gracefully
            exit 1
        }
...
If (($null -Ne $Installed) -And (-Not ($Installed.IsDevelopmentMode))) {
    Write-Warning "There is already one installed WSA. Removing it first..."
...
ElseIf ($null -Ne $Installed) {
    Write-Output "Failed to update. Attempting clean install..."
